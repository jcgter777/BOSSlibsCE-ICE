BOSshell CE libs for ICE by beckadamtheinventor, version 1.02.01 (professional edition)
...
To use the libraries, simply 'include' them in the header of your program with this line of code: "AsmComp(BOSSLIBP", which automatically sets up the full-screen mode, closes all open files, and displays a 'splash-screen'.
Commands using 'Call' ('maxY' when ICE is not installed):
	...
	IMGH
	: draw image from hex colors, uses list IDAT as image.
	: draws at X,Y and with scale factor XS,YS. Length X,Y is first two indexes of IDAT, rest is image data.
	IMGB
	: draw image from basic colors, must have initialized BASIC colors first, but you can setup these manually by assigning the list COLORS with the pallette you want to use, uses IDAT as image. ALSO: you need a space at the begginning so that ICDE doesn't 'squish' the string to hex.
	: draws at X,Y and with scale factor XS,YS. Length X,Y is first two indexes of IDAT, rest is image data.
	GIFH
	: draw 'gif' image sequence, uses list IDAT as image.
